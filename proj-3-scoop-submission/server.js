...
function createComment(url, request){
  ...
  const requestComment = request.body.comment; // && request.body.usrname && request.body.articleId
...
function deleteComment(url, request){
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const commentKeys = Object.keys(database.comments);
  const response = {};

  if (commentKeys.includes(id.toString())) {
    const savedComment = database.comments[id];
    const artId = savedComment.articleId;
    const savedArticle = database.articles[artId];
    const thisUser = database.users[savedComment.username];

    database.comments[id] = null;
    if (thisUser.commentIds.includes(id)){
      thisUser.commentIds.splice(thisUser.commentIds.indexOf(id), 1);
    }
    if (savedArticle.commentIds.includes(id)){
      savedArticle.commentIds.splice(savedArticle.commentIds.indexOf(id), 1);
    }
    response.status = 204;
  } else {
    response.status = 404;
...
function loadDatabase(){
  const config = yaml.safeLoad(fs.readFileSync('yamldb.yml', 'utf8'));
  database = config;
}

function saveDatabase(){
  var yaml = require('write-yaml');
  yaml.sync('yamldb.yml', database);
}
...
const getRequestRoute = (url) => {
  ...
  if (typeof qa  === 'function' && !isTestMode) {
